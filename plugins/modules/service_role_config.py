...
class ClusterServiceRoleConfig(
    ...
    def process(self):
        ...
        try:
            existing = api.read_role_config(self.cluster, self.role, self.service)
        ...
        if change_set:
            ...
            if not self.module.check_mode:
                ...
                self.output["config"] = [
                    p.to_dict()
                    for p in api.update_role_config(
                        self.cluster,
                        self.role,
                        self.service,
...
def main():
    ClusterServiceRoleConfig().execute()
